...
author: "Blandine Lyonnard, Camila Pacheco Riaño & Katrín Björnsdóttir"
date: "2026-04-15"
...
itex0 <- read_csv("Data/ITEX_diversity_data.csv") %>% 
  dplyr::select(-...1) # this code takes out the first column that was called "...1", it's not necessary though, it just makes the dataframe cleaner.
...
How does climate change (i.e. temperature) impacts a variable of interest (i.e. diversity) across sites?

In other words: What is the relationship between diversity and temperature across sites (i.e. across latitude)?

```{r}
itex <- itex0 %>% 
  mutate(TempC = (PlotTemp - 30)/2) #change temperature from F to °C (optional)
```
...
-   The estimate, for TempC, is the slope of the relationship (0.14623)

-   Pr is the p value from the model - in our case it is significant since it is lower than 0.05.

-   Adjusted R-squared tells us how much variation was explained by the model - in our case 0.1057 or 10%.
...
```{r}
mixed.lmer <- lmer(SppRich ~ TempC + (1|SUBSITE/PLOT), data = itex)
summary(mixed.lmer)
```
...
Let's look for missing values then

```{r}
sum(is.na(itex$SppRich)) # no NAs in SppRich, how about TempC?
```
...
```{r, warning = FALSE}
itex <- itex %>% 
  filter(!is.na(TempC)) # here we filter out all NA values within TempC
```

*sidenote: adding a "!" in front will remove values when using the filter function*
...
The summary output for linear mixed models in the lme4 package don't show p-values.This is a conscious choice made by the authors of the package, as there are many problems with p-values. 
...
Always choose variables based on biology/ecology and avoid adding in all possible variables that you have access to (i.e. don’t overfit). Remember that as a rule of thumb, you need 10 times more data than parameters you are trying to estimate. 
...
You can also use a package called lmerTest and run your models and the summary again with this package loaded and the p-values will be added to the summary.
...
[blandine.lyonnard\@bioenv.gu.se](mailto:blandine.lyonnard@bioenv.gu.se){.email}
