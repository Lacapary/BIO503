...
author: "Katrín Björnsdóttir, Blandine Lyonnard, Camila Pacheco Riaño"
date: "2026-04-27"
format: html
editor: visual
execute:
...
# 2. Multivariate analysis exercise
...
Functions are used in R to perform a task you would otherwise have to repeat several times by copy pasting your code and changing it slightly. Or to perform a specific task that does not have a pre-build function in R base or in any package.

First you define the function (in our case NMDS.scree) and then later on you can call that function to perform your task.

```{r}
NMDS.scree <- function(x) { # where x is expected to be the data frame or matrix used as input
  plot(rep(1, 5), 
       replicate(5, metaMDS(x, autotransform = F, k = 1)$stress), 
       xlim = c(1, 5),
       ylim = c(0, 0.30), 
       xlab = "# of Dimensions",
       ylab = "Stress", 
  ...
  for (i in 1:10) {
    points(rep(i + 1, 5), 
           replicate(5, metaMDS(x, autotransform = F, k = i + 1)$stress))
  }
}
```
...
We perform this to evaluate the optimal number of dimensions we will use in our analysis. Remember that we want our analysis to have as low stress as possible but also if we can get away with low number of dimensions that is good. So we will choose the number of dimensions depending on the difference in stress level between dimensions.
...
```{r, eval = FALSE, message = FALSE}
NMDS2 <- metaMDS(itex.nmds, k = 2, trymax = 100, trace = F, 
                 autotransform = FALSE, 
                 distance = "bray")
plot(NMDS2)
...
[blandine.lyonnard\@bioenv.gu.se](mailto:katrin.bjornsdottir@bioenv.gu.se){.email}
