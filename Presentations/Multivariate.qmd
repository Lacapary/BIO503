...
# Load the community dataset which we`ll use in 
# the examples today
...
-   The **community as a whole** responds, not just individual species.
-   Running one univariate model per species inflates the risk of false positives and ignores covariance between species.
...
knitr::include_graphics(normalizePath("sources/Diaz_etal.png"))
...
Before we can cluster, we need to quantify **how different two sites are**.

-   **Bray-Curtis** → species abundance data. Ignores joint absences (two sites sharing *zero* individuals of a species are not automatically "similar").
-   **Jaccard** → presence/absence data.
-   **Euclidean** → continuous environmental variables (pH, temperature, moisture).
-   **Chi-square** → emphasizes relative composition; the metric behind Correspondence Analysis.

## Step 1 — Choose a distance metric

::: fragment
> 🌿 *Rule of thumb in community ecology*: use **Bray-Curtis** for species abundances.
:::

## Step 2 — Choose a linkage method

Once we have pairwise distances, how do we merge sites into groups?

-   **Single** → distance to the *nearest* member. Tends to produce long "chains".
-   **Complete** → distance to the *farthest* member. Produces compact clusters.
-   **Average (UPGMA)** → mean pairwise distance.
-   **Ward.D2** → minimizes within-cluster variance → balanced, ecologically interpretable clusters.

## Step 2 — Choose a linkage method

::: fragment
> We will use `ward.D2` with Bray-Curtis distances.
> This combination is common in vegetation studies because it produces clusters of similar size and variance.
:::

## Step 3 — Reading a dendrogram

![source: Prasad Pai](https://miro.medium.com/v2/resize:fit:640/format:webp/1*2MAGLkkfRXSXhQ9pEgK0WQ.png)

-   The **merge height** = dissimilarity between the clusters being merged.
-   **Lower heights** = very similar communities (share most species at similar abundances).
-   **Higher heights** = strongly different communities (different species pool).
...
plot(dend.plot, 
     ...
     main = "why would clusters be different?")
```
...
#| eval: true
#| echo: true
fviz_cluster(kmeans(varespec_m, centers = 2), 
             geom = "point", 
             data = dune2_spe) + 
  theme_minimal()
```
...
-   We will focus on **Unconstrained Ordination**, which arranges samples or species continuously along latent gradients without using any external predictor.
...
## Species response along gradients

Before picking a method, ask: **how do species respond to the underlying gradient?**

-   **Linear response** → a species' abundance steadily increases (or decreases) along the gradient. Typical for **short** gradients (e.g. a narrow range of pH).
-   **Unimodal response** → a species peaks at its optimum and declines on either side. Typical for **long** gradients where species turn over (different species at each end).
-   **Short gradient, linear response** → PCA.
-   **Long gradient, unimodal response, high turnover** → DCA or NMDS.

## Species response along gradients

![source: davidzeleny](https://www.davidzeleny.net/anadat-r/lib/exe/fetch.php/obrazky:double-zero-illustration.png)
...
#| eval: true
#| echo: true
PCA <- rda(dune2_spe, scale = FALSE) # Use scale = TRUE 
# if your variables are on different scales 
# (e.g. for abiotic variables).
...
# that point in the direction of increasing values 
# for that variable
biplot(PCA, choices = c(1,2), type = c("text", "points"), 
       xlim = c(-5,5)) # biplot of axis 1 vs 2
```
...
## Environmental Variables and Triplot
...
```{r, echo=FALSE, out.width="60%"}
knitr::include_graphics("sources/arch_effect.png")
```
:::

## Detrended correspondence analysis (DCA)

Arch can be removed by detrending (*smoothing out the data to make it easier to see the main patterns*), which is the base of the detrended correspondence analysis (DCA).
...
-   So there's no unique solution (you might get slightly different results each time — always set a seed).
-   Axes are not ranked by explained variance — they are just an arbitrary reference frame. You can rotate the plot freely.
-   You must choose the number of dimensions `k` in advance.

## Interpreting stress

Stress measures how well the reduced-space distances reflect the original dissimilarities.

| Stress      | Interpretation                                       |
|-------------|------------------------------------------------------|
| \< 0.05     | Excellent representation                             |
| 0.05 – 0.10 | Good, reliable                                       |
| 0.10 – 0.20 | Usable, interpret with care                          |
| \> 0.20     | Suspicious — try more dimensions or another distance |

## Non-metric Multidimensional Scaling (NMDS)

Methodology of NMDS:

1.  Perform NMDS with 1 to 10 dimensions.
2.  Check the stress vs. dimension plot (the "scree" plot).
...
# See PCOA for more information about the distance measures
# Here we use bray-curtis distance, which is recommended for 
# abundance data
dist <- vegdist(dune2_spe,  method = "bray")

# In this part, we define a function NMDS.scree() 
# that automatically performs a NMDS for 1-10 dimensions 
# and plots the nb of dimensions vs the stress

NMDS.scree <- function(x) { # where x is expected to be 
  # the data frame or matrix used as input
  plot(rep(1, 10), 
...
# If you don`t provide a dissimilarity matrix, metaMDS 
# automatically applies Bray-Curtis. So in our case, 
# the results would have to be the same
...
1.  **What kind of data do I have?**
    -   Species abundances → Bray-Curtis distance, NMDS is a safe default.
    -   Presence/absence → Jaccard distance, NMDS or DCA.
    -   Continuous env. variables → Euclidean distance, PCA.
2.  **Do I have hypotheses about environmental drivers?**
    -   Yes → **constrained** ordination (RDA if linear, CCA if unimodal).
    -   No → **unconstrained** (PCA / DCA / NMDS).
3.  **Are my data messy, non-normal, or zero-inflated?**
    -   NMDS is the most robust choice.

## Summary

::: fragment
> 🧭 *Clustering and ordination are complementary*: clustering tells you **which** sites group together, ordination tells you **along what gradient** they differ.
:::
...
::: fragment
```{r, echo=TRUE}
length(unique(itex.long$SITE)) 
# this code tells you how many different samples we have 
# within the SITE column
```
:::

::: fragment
```{r, echo=TRUE}
unique(itex.long$SITE) 
# you can also do this and then it gives you the name 
# of all samples within the SITE column
```
:::
